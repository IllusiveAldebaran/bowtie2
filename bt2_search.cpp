		   ...
		   */
			...
			if(rdid >= skipReads && rdid < qUpto && sample) {
				...
				while(retry) {
					...
					prm.nFilt += (filt[0] ? 0 : 1) + (filt[1] ? 0 : 1); //bruh, I know they're boolean... but it goes from boolean to numbers? Or is this parsed to true and false? It is parsed into addition you fool. Sorry... okay.. I see. It's an accumulator... interesting... I don't quite follow how these filters act on the code.
					...
					// TRYUP3
					if(filt[0] && filt[1]) { // filt[1] is true, try to just put filt[0]
					...
					bool done[2] = { !filt[0], !filt[1] }; // by default filt is true
					size_t nelt[2] = {0, 0};
...
//`					bool paired = !ps->read_b().empty(); // what a cheap way to check... shouldn't this just be done way earlier?
//`					const size_t rdlen1 = ps->read_a().length();
//`					const size_t rdlen2 = paired ? ps->read_b().length() : 0; // DIFF
//`					olm.bases += (rdlen1 + rdlen2);
//`					msinkwrap.nextRead(
//`						&ps->read_a(),
//`						paired ? &ps->read_b() : NULL, // PAIR
//`						rdid,
//`						sc.qualitiesMatter());
//`					assert(msinkwrap.inited());
//`					size_t rdlens[2] = { rdlen1, rdlen2 };
...
//`						float T = bwaSwLikeT, c = bwaSwLikeC;
//`						minsc[0] = (TAlScore)max<float>(a*T, a*c*log(rdlens[0]));
//`						if(paired) { // DIFF I guess this is comparing two. rdlesn is just hte lengths of both.
//`							minsc[1] = (TAlScore)max<float>(a*T, a*c*log(rdlens[1]));
//`						}
//`					} else { // I assume this is bwa and not bwaswlike. Also, seems bwa-sw is one of three core algorithms in bwa. Check https://github.com/lh3/bwa
//`						minsc[0] = scoreMin.f<TAlScore>(rdlens[0]);
//`						if(paired) minsc[1] = scoreMin.f<TAlScore>(rdlens[1]); //DIFF
//`						// from my understanding rdlens[1] =0... so I assume whatever this line above does will just = 0, but idk what it is fully doing.... where else would it define minsc[1]? Actually.. a couple lines above before the if(bwaSwLike) statement... lol... okay so it sets a huge number by default then.
//`						if(localAlign) {
//`							if(minsc[0] < 0) {
//`								if(!gQuiet) printLocalScoreMsg(*ps, paired, true);
...
//`						lenfilt[0] = false;
//`					}
//`					if((rdlens[1] <= (size_t)multiseedMms || rdlens[1] < 2) && paired) { // DIFF
//`						if(!gQuiet) printMmsSkipMsg(*ps, paired, false, multiseedMms);
//`						lenfilt[1] = false;
//`					}
//`					if(rdlens[0] < 2) {
...
//`					// Calculate interval length for both mates
//`					int interval[2] = { 0, 0 };
//`					for(size_t mate = 0; mate < (paired ? 2:1); mate++) { // DIFF look happens differently depending on stuff
//`						interval[mate] = msIval.f<int>((double)rdlens[mate]);
//`						if(filt[0] && filt[1]) {
//`							// Boost interval length by 20% for paired-end reads
//`							interval[mate] = (int)(interval[mate] * 1.2 + 0.5);
...
//`					assert_gt(nrounds[0], 0);
//`					// Increment counters according to what got filtered
//`					for(size_t mate = 0; mate < (paired ? 2:1); mate++) { // DIFF
//`						if(!filt[mate]) {
//`							// Mate was rejected by N filter
//`							olm.freads++;               // reads filtered out
//`							olm.fbases += rdlens[mate]; // bases filtered out
...
//`						//	if(seedlens[1] > 8) seedlens[1]--;
//`						//}
//`						for(size_t matei = 0; matei < (paired ? 2:1); matei++) { // DIFF 
//`							size_t mate = matemap[matei];
//`							if(done[mate] || msinkwrap.state().doneWithMate(mate == 0)) {
//`								// Done with this mate
//`								done[mate] = true;
